    ...
    return (
        <div className="min-h-screen p-4 flex flex-col items-center justify-center bg-paper">
            <div className="w-full max-w-5xl">
                <h1 className="text-3xl md:text-5xl font-bold font-doodle text-center mb-8 transform -rotate-1">
                    Setup Your Game 🛠️
                </h1>

                {/* Stepper */}
                <div className="flex justify-center mb-8 gap-2 md:gap-4">
                    <div className={`px-3 py-1 md:px-4 md:py-2 border-2 border-black rounded-full font-bold text-sm md:text-base ${step === 1 ? 'bg-yellow-300 shadow-[4px_4px_0px_black]' : 'bg-white opacity-50'}`}>
                        1. Choose Map 🗺️
                    </div>
                    <div className={`px-3 py-1 md:px-4 md:py-2 border-2 border-black rounded-full font-bold text-sm md:text-base ${step === 2 ? 'bg-green-300 shadow-[4px_4px_0px_black]' : 'bg-white opacity-50'}`}>
                        2. Settings ⚙️
                    </div>
                </div>
                ...
                <div className="min-h-[400px]">
                    {step === 1 && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                            ...
                            {quizzes.map(quiz => (
                                ...
                                >
                                    <div className="text-4xl md:text-6xl mb-4">{quiz.thumbnail || '📝'}</div>
                                    <h3 className="text-xl md:text-2xl font-bold font-doodle mb-2">{quiz.title}</h3>
                                    <p className="text-gray-600 mb-2 text-sm md:text-base">{quiz.description}</p>
                    ...
                    {step === 2 && (
                        <div className="max-w-2xl mx-auto doodle-card bg-white rotate-1">
                            <h2 className="text-2xl md:text-3xl font-bold font-doodle mb-8 text-center">Game Rules</h2>

                            <div className="space-y-8">
                                {/* Timer Setting */}
                                <div>
                                    <label className="block text-lg md:text-xl font-bold font-doodle mb-4">⏱️ Time Limit</label>
                                    <div className="flex gap-2 md:gap-4 justify-center flex-wrap">
                                        {[10, 20, 30, 0].map(val => (
                                            <button
                                                key={val}
                                                onClick={() => setSettings({ ...settings, timeLimit: val })}
                                                className={`px-4 md:px-6 py-2 md:py-3 border-2 border-black rounded-lg font-bold text-base md:text-lg transition-all ${settings.timeLimit === val
                                ...
                                {/* Host Mode Setting */}
                                <div>
                                    <label className="block text-lg md:text-xl font-bold font-doodle mb-4">🎮 Your Role</label>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                                        <button
                                            onClick={() => setSettings({ ...settings, isHostPlaying: true })}
                                            className={`p-4 md:p-6 border-2 border-black rounded-xl text-left transition-all ${settings.isHostPlaying
                                                ? 'bg-green-300 shadow-[4px_4px_0px_black] transform -translate-y-1'
                                                : 'bg-white hover:bg-gray-100 opacity-60'
                                                }`}
                                        >
                                            <div className="text-4xl mb-2">🏃‍♂️</div>
                                            <div className="font-bold text-lg md:text-xl">Player</div>
                                            <div className="text-xs md:text-sm">I want to answer questions too!</div>
                                        </button>
                                        ...
                                        >
                                            ...
                                            <div className="font-bold text-lg md:text-xl">Host Only</div>
                                            <div className="text-xs md:text-sm">I'll just watch and control the game.</div>
                                        </button>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="flex justify-between mt-12 max-w-4xl mx-auto w-full">
                    ...
                    {step === 1 ? (
                        ...
                        <button
                            ...
                            disabled={loading}
                            className="doodle-button bg-green-400 text-white text-lg md:text-2xl px-8 md:px-12 transform rotate-1"
                        >
